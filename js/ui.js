  ...
  todos.forEach(t => {
    ...
    html += `
      <div class="todo-item" data-id="${t.id}">
        <button class="todo-check" data-action="toggle-todo" data-id="${t.id}" title="点击两次完成">✓</button>
        <div class="todo-body">
        ...
        <button class="todo-delete" data-action="delete-todo" data-id="${t.id}" title="删除">🗑</button>
      </div>`;
  });
  container.innerHTML = html;
}
...
function renderCompletedTodos() {
  ...
  if (completedCollapsed) { container.innerHTML = ''; return; }
