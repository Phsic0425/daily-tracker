...
function renderHelp() {
  ...
  h += '<div class="help-h3">💼 总资产 / 账户</div>';
  h += '<p>点「管理」→ + 添加账户（银行卡/现金/虚拟账户/一卡通等），设置初始余额。</p>';
  h += '<p>记一笔时可勾选对应账户（单选），选中后可切换记账方式：</p>';
  h += '<p>✏️ 输入变化量：和普通记账一样，直接输入本次收支金额。</p>';
  h += '<p>🎯 输入末状态值：输入账户变动后的最终余额，系统自动算出变化量并记账。</p>';
  h += '<p>账户余额会随每笔记账自动增减，顶部显示所有账户的总资产。</p>';
  ...
  h += '<div class="help-h3">📚 课表视图</div>';
  h += '<p>顶部「课表」按钮切换到课表视图（替代原周视图），按周一~周日 + 时间轴展示课程。</p>';
  h += '<p>「节次设置」设置每节课对应的起止时间。「+ 添加课程」填写名称、地点、星期、节次、上课周数。</p>';
  h += '<p>周数支持预设（全部周/单周/双周）或自定义输入，如 <code>1-8,10,12-16</code>。</p>';
  h += '<p>「设置周数」手动指定某天是第几周，之后系统按此锚点自动推算每周。</p>';
  h += '<p>本周有课高亮显示，非本周课程虚化显示（不会隐藏）。日程事件也会叠加显示在课表上，但课程本身不会出现在月历中。</p>';
  h += '<p>「节次设置」弹窗内可调整「课表竖向缩放」「课表横向缩放」两个滑块，整体放大/缩小时间轴或每日列宽；「局部时间段缩放」可框定任意时间区间（如午休）单独设置缩放倍数，与其他时段互不影响。</p>';
  ...
  h += '<p>如果不想用云端同步，也可以用 ⋯ → 📋 复制同步码 / 📌 粘贴同步码 手动传输（全部模块）。</p>';

  h += '<div class="help-h3">🧩 分模块同步</div>';
  h += '<p>⋯ → 分模块同步，可勾选只导出账单/待办/日程/账户/课表等某几类数据，生成的码只包含勾选的模块。</p>';
  h += '<p>导入时先「解析同步码」，再勾选实际要导入的模块。<b>导入是整体覆盖，不是合并</b>——比如导入待办会把本设备的待办列表整体替换成码里的内容，不会跟本设备已有的待办合并去重。</p>';
  h += '</div>';
